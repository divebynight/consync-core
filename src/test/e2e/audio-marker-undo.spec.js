...
async function createUntitledMarker(window) {
...
test("removes the most recently created marker with Cmd+Z in the audio workspace", async () => {
  ...
  try {
    ...
    await createUntitledMarker(window);
    await createUntitledMarker(window);

    const timelineMarkersHeading = window.getByRole("heading", { name: "Timeline Markers" });
    const timelineMarkersSection = timelineMarkersHeading.locator("..");
    const untitledMarkerButtons = timelineMarkersSection.getByRole("button", { name: "Seek to marker Untitled marker" });

    await expect(untitledMarkerButtons).toHaveCount(2);

    await window.keyboard.press("Meta+z");

    await expect(untitledMarkerButtons).toHaveCount(1);
    await expect(timelineMarkersSection.getByText("Untitled marker")).toHaveCount(1);
  } finally {
