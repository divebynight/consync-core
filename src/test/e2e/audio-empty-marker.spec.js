...
async function createEmptyMarker(window) {
...
test("preserves empty markers and allows the next hotkey marker after Enter", async () => {
  ...
  try {
    ...
    await createEmptyMarker(window);
    ...
    await expect(timelineMarkersSection.getByText("Untitled marker")).toHaveCount(2);
  } finally {
