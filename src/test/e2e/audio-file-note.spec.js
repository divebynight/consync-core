...
test("creates a file note that appears in the File Notes section and not in Timeline Markers", async () => {
  ...
  try {
    ...
    // Fill the note input and click Add Note (not Save note at current time).
    const noteInput = window.getByLabel("Note text");
    await noteInput.fill("general observation");
    await window.getByRole("button", { name: "Add Note" }).click();

    // The note input should clear after saving.
    await expect(noteInput).toHaveValue("");

    // The note should appear in the File Notes section.
    const fileNotesSection = window.getByRole("heading", { name: "File Notes" }).locator("..");
    await expect(fileNotesSection.getByText("general observation")).toBeVisible();

    // File notes show "File note" as their time label (no timestamp).
    await expect(fileNotesSection.locator(".bookmark-time").first()).toHaveText("File note");

    // The note should NOT appear in the Timeline Markers section.
    const timelineMarkersSection = window.getByRole("heading", { name: "Timeline Markers" }).locator("..");
    await expect(timelineMarkersSection.getByText("general observation")).not.toBeVisible();
  } finally {
