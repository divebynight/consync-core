...
- Worked repair-entry example: package N ends `FAIL`, `package_plan.md` still identifies package N as the blocked planned package, and repo status or `handoff.md` shows unresolved work; result = enter repair, record the repair package, and set sequence status to `PAUSED_REPAIR`.
- Worked repair-return example: the repair package closes `PASS`, its executed instruction is archived, repo status is clean, resume-state classification returns `CLEAN`, and the blocked planned package is still clearly named in `package_plan.md`; result = return to that blocked planned package.
- Worked stop-before-return example: the repair package closes but repo status remains dirty, a new contradiction appears, or the blocked package can no longer be identified confidently; result = stop and inspect manually instead of returning to planned work.
