...
- `.scaffoldai/agents/` is the source of truth for invoked-agent roles and invocation order.
- `.scaffoldai/agents/entry-adapter.agent.md` defines the manual Phase 2 input-classification adapter for recommending which existing agent a human should invoke next.
...
The Phase 2 Entry Adapter lives at `.scaffoldai/agents/entry-adapter.agent.md`.
