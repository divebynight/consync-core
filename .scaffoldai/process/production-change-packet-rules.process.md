...
Follow `.scaffoldai/templates/work-packet-v3.md` for the generic packet lifecycle, outcomes, ALLOWED FILES, commit rules, and output contract. The safeguards below are the extra requirements for packets that modify production code.
...
Confirm the focused spec passes before running the full suite. The e2e count must increase by the expected amount when a new spec is added. Verification level definitions live in `.scaffoldai/verification/verification-ladder.md`.

If FULL_VERIFY fails, stop and record STOPPED before attempting anything else. Generic STOPPED handling is defined in `.scaffoldai/templates/work-packet-v3.md`.

### Production-specific documentation

- Append an entry to `.scaffoldai/process/work-log.log.md` describing the production behavior and verification result
- Update `.scaffoldai/verification/ui-e2e-coverage-map.md` if a new spec was added
