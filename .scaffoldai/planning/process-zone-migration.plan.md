...
| `src/lib/intakeClassify.agent.scaffoldai.js` | String update required | `.scaffoldai/process/`, `.scaffoldai/agents/`, `.consync/docs/` appear as classification pattern strings — not file reads |
...
| `src/commands/handoff-bundle.process.scaffoldai.js` | Path update required | Contains `RUNBOOK_PATH = path.join(".consync", "process", "runbook.process.md")` — PROCESS pointer stored as a constant |
...
3. Update all path references in the same commit:
   ...
   - `src/lib/intakeClassify.agent.scaffoldai.js` (classification strings)
   - `src/commands/reference-audit.js` (zone needle strings)
   - `src/commands/handoff-bundle.process.scaffoldai.js` (`RUNBOOK_PATH` constant)
...
| Stale `intakeClassify.agent.scaffoldai.js` patterns | LOW | Classification strings are not filesystem reads; stale patterns produce wrong classification labels but do not crash |
