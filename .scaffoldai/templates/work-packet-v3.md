...
Before doing any work, check the following. If ALL pass, return STATUS: ALREADY_COMPLETE.
  ...
  3. Work-log entry exists:
     - .scaffoldai/process/work-log.log.md contains an entry for this task or PACKET_ID
...
Only enter this path if ALREADY_COMPLETE CHECK fails on one or more items.
  ...
  DOCUMENT:
  - Append an entry to .scaffoldai/process/work-log.log.md describing what was done.
...
See `.scaffoldai/verification/verification-ladder.md` for the full verification level definitions.
See `.scaffoldai/skills/closeout-agent.md` for the closeout workflow.
See `.scaffoldai/process/production-change-packet-rules.md` for rules governing packets that modify production source.
